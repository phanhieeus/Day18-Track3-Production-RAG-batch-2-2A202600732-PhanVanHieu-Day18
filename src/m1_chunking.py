...
def chunk_hierarchical(text: str, parent_size: int = HIERARCHICAL_PARENT_SIZE,
    ...
    for para in paragraphs:
        if len(current) + len(para) > parent_size and current:
            _add_parent(parents, children, current, child_size, metadata)
            current = ""
        current += para + "\n\n"
    if current.strip():
        _add_parent(parents, children, current, child_size, metadata)
...
def _add_parent(parents, children, text, child_size, metadata):
    """Tạo 1 parent chunk + cắt nó thành các child chunk (mỗi child ≤ child_size)."""
    ...
    for word in text.split():
        if len(buf) + len(word) + 1 > child_size and buf:
